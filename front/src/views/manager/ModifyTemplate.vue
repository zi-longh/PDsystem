<template>
  <div>
    <div class="card" style="margin-bottom: 10px;">
      <p style="margin: 5px;">
        <span style="font-size: 24px; font-weight: bold">修改模板信息 </span>
         <el-select
            v-model="data.templateValue"
            clearable
        ...
        <span style="margin-left: 15px">当前查看的模板名称：{{ data.newValue }}</span>
      </p>
      <span style="margin-left: 15px; font-style: italic; color: #888; font-size: small"> 说明：下方修改模板基本信息，设定的格式规范，最后点击右侧“修改”按钮修改您的论文模板。 </span>
      ...
      <p style="margin-left: 5px; margin-bottom: 1px">
        <el-form :model="data.baseData" :inline="true" :rules="rules" label-width="auto" >
          ...
          <el-form-item>
            <el-button type="primary" style="margin-right: 10px;" @click="ModifyTemplate">修改模板信息</el-button>
          </el-form-item>
        </el-form>
      </p>
    </div>
...
const data = reactive({
  ...
  templateData: {
    ...
    pagesettingTopmargin: '1417',
    pagesettingBottommargin: '1417',
    pagesettingLeftmargin: '1701',
    pagesettingRightmargin: '1134',
    pagesettingHeadermargin: '850',
    pagesettingFootermargin: '1417',
...
const ModifyTemplate = () => {
  let x = data.templateValue.split("-")[0].toString();
  request.post('/updateTemplate', {
    templateId: x,
    creator: data.baseData.creator,
    templateName: data.baseData.templateName,
    description: data.baseData.description,
    ...
    if (res.code === '200') {
      request.post('/updateTemplateInfo', data.templateData).then(res => {
        if (res.code === '200') {
...
const loadTemplateData = () => {
  let x = data.templateValue.split("-")[0].toString();
  request.get('/getTemplateInfoById2', {
    params: {
      template_id: x
    }
  }).then(res => {
  ...
  request.post('/getTemplateById',{
    templateId: x
  }).then(res => {
    data.baseData.templateName = res.data.templateName;
    data.baseData.description = res.data.description;
    data.baseData.status = res.data.status == '1'? true : false;
  })
  data.newValue =  data.templateValue;
};

const load = () => {
  // 如果有数据
  let templateId = localStorage.getItem("templateId-my");
  let templateName = localStorage.getItem("templateName-my");
  if (templateId) { // 如果有值
    data.templateValue = templateId + "-" + templateName;
    loadTemplateData();
  }
  ...
  request.get('/getTemplateListForTeacher').then(res => {
    // 筛选出当前用户的模板
    options.templates = res.data.filter(item => item.creator === user.username)
  });
